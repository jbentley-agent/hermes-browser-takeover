...
const os = require('os');
const { expandMacro } = require('@askjo/camoufox-browser/lib/macros');
...
function normalizeCamoufoxVirtualDisplay(options) {
  const screenWidth = coercePositiveInt(process.env.SCREEN_WIDTH, 1440);
  const screenHeight = coercePositiveInt(process.env.SCREEN_HEIGHT, 900);
  const screenAvailHeight = coercePositiveInt(process.env.SCREEN_AVAIL_HEIGHT, Math.max(screenHeight - 32, 1));
  const camouConfigKey = Object.keys(options.env || {}).find((key) => key.startsWith('CAMOU_CONFIG_'));

  if (camouConfigKey) {
    try {
      const camouConfig = JSON.parse(options.env[camouConfigKey]);
      Object.assign(camouConfig, {
        'screen.width': screenWidth,
        'screen.height': screenHeight,
        'screen.availWidth': screenWidth,
        'screen.availHeight': screenAvailHeight,
        'window.outerWidth': screenWidth,
        'window.outerHeight': screenAvailHeight,
        'window.screenX': 0,
        'window.screenY': 0,
      });
      options.env[camouConfigKey] = JSON.stringify(camouConfig);
    } catch (err) {
      console.warn(`Failed to normalize ${camouConfigKey}: ${err.message}`);
    }
  }
  ...
  options.args = [
    ...(options.args || []).filter((arg) => !arg.startsWith('--width=') && !arg.startsWith('--height=')),
    `--width=${screenWidth}`,
    `--height=${screenHeight}`,
  ];

  console.log(`Normalized Camoufox display to ${screenWidth}x${screenHeight} (availHeight=${screenAvailHeight})`);
}
...
async function ensureBrowser() {
  if (!browser) {
    ...
    // Load CapSolver Firefox addon for auto CAPTCHA solving if available
    const capsolverAddonPath = process.env.CAPSOLVER_ADDON_PATH || '/root/capsolver-firefox-addon';
    const addonsList = [];
    try {
      const fs = require('fs');
      if (fs.existsSync(capsolverAddonPath + '/manifest.json')) {
        addonsList.push(capsolverAddonPath);
        console.log(`Loading CapSolver addon from ${capsolverAddonPath}`);
      }
    } catch (e) { /* addon not available, skip */ }
    ...
    normalizeCamoufoxVirtualDisplay(options);
